...
namespace Nox.Avatars.Runtime.client {
	...
	public class AvatarComponent : MonoBehaviour {
		...
		private async UniTask OnSelectClickedAsync() {
			if (!selectButton.interactable || Page?.Avatar == null)
				return;
		...
		public static (GameObject, AvatarComponent) Generate(AvatarPage avatarPage, RectTransform parent) {
			...
			var profile = Client.GetAsset<GameObject>("ui:prefabs/profile.prefab")
			...
			// generate dashboard
			container = Client.GetAsset<GameObject>("ui:prefabs/container_full.prefab")
				.Instantiate(splitContent);
			...
			var labelObj   = labelAsset.Instantiate(Reference.GetComponent<RectTransform>("content", header));

			Reference.GetComponent<Image>("image", icon)
				.sprite = Client.GetAsset<Sprite>("ui:icons/avatar.png");
			...
			// add box actions
			var boxActions = boxAsset.Instantiate(component.content);
			...
			component.actions = Reference.GetComponent<RectTransform>(
				"content",
				actionContainerAsset.Instantiate(Reference.GetComponent<RectTransform>("content", boxActions))
			);
			...
			// add box description
			component.descriptionContainer = boxAsset.Instantiate(component.content);
			...
			component.descriptionText = Reference.GetComponent<TextLanguage>(
				...
				Client.GetAsset<GameObject>("ui:prefabs/text.prefab")
					.Instantiate(Reference.GetComponent<RectTransform>("content", component.descriptionContainer))
			);
		...
		private static void GenerateActions(AvatarComponent component) {
			...
			var select             = actionButtonAsset.Instantiate(component.actions);
			...
			var cache             = actionButtonAsset.Instantiate(component.actions);
			...
			var favorite             = actionButtonAsset.Instantiate(component.actions);
